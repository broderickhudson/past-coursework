...
class CourseDocumentsClass
{
		...
		for (int index = 1; index < array.length; index++)
		{
			for (int matchCheck = index+1; matchCheck < array.length; matchCheck++)
			{
					...
					!(array[matchCheck][COURSE_ID].equals("NO") || array[index][COURSE_ID].equals("NO")))
				{
					...
					// Deal with any possible / in the numbers, cast them to strings, and sum them
					int enrollment;
					if (enrollString.contains("/"))
					{
						enrollment = splitSum(enrollString);
					}
					else
					{
						enrollment = Integer.parseInt(enrollString);
					}
	...
	{	
		...
		String[][] finalJoinCourse = condenseLinesTime(joinCourse);
		...
		// Use assembleCSVtoArray to create an array containing the contents of CourseFinal.txt
		String[][] courseFinal = assembleCSVtoArray("CourseFinal.txt");
		
		// Use condenseLinesID to get an array with all lines sharing class ID and instructor condensed into 1
		String[][] trueCourseFinal = condenseLinesID(courseFinal);
		
		// Use assembleArraytoCSV to output trueCourseFinal into a file called CourseFinal4.txt
		assembleArraytoCSV(trueCourseFinal, "CourseFinal4.txt");
